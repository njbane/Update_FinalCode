...
void recipe_cmd(uint8_t ID[], int board, int Serial){
	...
	*/
	...
	char buffer[15]; // Buffer for UART transmission
	...
	while(quanity > 0){ // Allows multiple drinks to be made
		...
		for(int i = 0; i <=7; i++){ 
			...
			**/
			...
			begin_time1 = HAL_GetTick(); // Gets current time in milliseconds for pumps
			while((stop_flag1*stop_flag2*stop_flag3*stop_flag4) != 1){ 
				...
				**/
				if(Serial == 1){
					len = sprintf(buffer, "ForLOOP");
					HAL_UART_Transmit(&huart2,(uint8_t*)buffer,len,1000);
					HAL_UART_Transmit(&huart2,(uint8_t*) i,1,1000);
				}
...
//				}
				if(REGA1.pin[i] == 1){
					if(((int)HAL_GetTick - begin_time1) > (117*((int) recipe[(int) ID[0]].time[i]))){
						/*
				...
				if(REGB1.pin[i] == 1){
					if(((int)HAL_GetTick - begin_time1) > (117*((int) recipe[(int) ID[0]].time[i+8]))){
						/*
						((int)HAL_GetTick - begin_time1): [TIME ELAPSED] Gets current time in microseconds and subtracts beginning time
				...
				if(board == 1){
					if(REGA2.pin[i] == 1){
						if(((int)HAL_GetTick - begin_time1) > (117*((int) recipe[(int) ID[0]].time[i+16]))){
							/*
							((int)HAL_GetTick - begin_time1): [TIME ELAPSED] Gets current time in microseconds and subtracts beginning time
					...
					if(REGB2.pin[i] == 1){
						if(((int)HAL_GetTick - begin_time1) > (117*((int) recipe[(int) ID[0]].time[i+24]))){
							/*
							((int)HAL_GetTick - begin_time1): [TIME ELAPSED] Gets current time in microseconds and subtracts beginning time
